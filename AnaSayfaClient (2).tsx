...
export default function AnaSayfaClient({ initialIlanlar, ilkGorsel }: Props) {
  ...
  useEffect(() => {
    if (initialIlanlar.length === 100) {
      const veriCek = async () => {
        try {
          const res = await fetch("/api/ilanlar?limit=500");
          if (res.ok) {
            const data = await res.json();
            if (data.ilanlar?.length > 100) setIlanlar(data.ilanlar);
          }
        } catch {}
      };
      veriCek();
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-btn { width: 38px; height: 38px; border-radius: 10px; border: 1.5px solid var(--border); background: #fff; color: var(--mid); font-family: inherit; font-size: .85rem; font-weight: 700; cursor: pointer; transition: .18px; display: flex; align-items: center; justify-content: center; }
