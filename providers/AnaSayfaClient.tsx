...
export default function AnaSayfaClient({ initialIlanlar }: { initialIlanlar: any[] }) {
  ...
  // Arka planda kalan ilanları çek
  useEffect(() => {
    ...
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-logo-icon { background: var(--red); width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: .95rem; flex-shrink: 0; box-shadow: 0 0 0 3px rgba(232,54,26,.3); }
        ...
        .hero::before { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse at 20% 50%, rgba(245,166,35,.08) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(37,99,235,.15) 0%, transparent 60%); pointer-events: none; }
        ...
        .hero-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(245,166,35,.15); border: 1px solid rgba(245,166,35,.3); color: var(--gold); padding: 6px 16px; border-radius: 99px; font-size: .78rem; font-weight: 700; margin-bottom: 20px; letter-spacing: .05em; }
        ...
        .badge { font-size: .62rem; font-weight: 800; padding: 3px 9px; border-radius: 7px; text-transform: uppercase; letter-spacing: .05em; box-shadow: 0 2px 8px rgba(0,0,0,.15); }
        ...
        .destek-fab { position: fixed; bottom: 90px; right: 20px; z-index: 300; width: 52px; height: 52px; border-radius: 50%; background: var(--green); border: none; box-shadow: 0 6px 24px rgba(16,185,129,.45); color: #fff; font-size: 1.4rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: .22s; animation: pulse 3s ease-in-out infinite; }
        .destek-fab:hover { transform: scale(1.1); }
        @keyframes pulse { 0%,100% { box-shadow: 0 6px 24px rgba(16,185,129,.45); } 50% { box-shadow: 0 6px 32px rgba(16,185,129,.7), 0 0 0 8px rgba(16,185,129,.15); } }
        .destek-panel { position: fixed; bottom: 154px; right: 20px; z-index: 300; width: 320px; background: #fff; border-radius: 20px; border: 1.5px solid var(--border); box-shadow: 0 16px 48px rgba(0,0,0,.15); overflow: hidden; animation: slideUp .25s ease; }
        ...
        .destek-send { width: 100%; margin-top: 10px; padding: 12px; border-radius: 12px; background: var(--navy); border: none; color: #fff; font-family: inherit; font-size: .9rem; font-weight: 700; cursor: pointer; transition: .2s; }
        ...
        /* ── ALT MENÜ ── */
        .bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; height: 64px; background: var(--navy); display: flex; align-items: stretch; z-index: 400; box-shadow: 0 -4px 20px rgba(0,0,0,.25); border-top: 1px solid rgba(255,255,255,.08); }
        .bottom-nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; cursor: pointer; border: none; background: transparent; color: rgba(255,255,255,.5); font-family: inherit; font-size: .62rem; font-weight: 700; transition: .18s; position: relative; padding: 0; text-transform: uppercase; letter-spacing: .03em; }
        .bottom-nav-item:hover { color: rgba(255,255,255,.85); }
        .bottom-nav-item.active { color: var(--gold); }
        .bottom-nav-item.active::after { content: ''; position: absolute; top: 0; left: 20%; right: 20%; height: 2px; background: var(--gold); border-radius: 0 0 4px 4px; }
        .bottom-nav-icon { font-size: 1.25rem; line-height: 1; }
        .bottom-nav-primary { background: var(--red) !important; color: #fff !important; margin: 8px 6px; border-radius: 12px !important; height: calc(100% - 16px); box-shadow: 0 4px 14px rgba(232,54,26,.4); }
        .bottom-nav-primary .bottom-nav-icon { font-size: 1.4rem; }
      ...
      <header className="hero">
        ...
        <div className="kontrol">
          <div className="toggle-row">
            <button className={`toggle-btn ${aktifTip === "bireysel" ? "on-bireysel" : ""}`} onClick={() => { setAktifTip("bireysel"); setAktifKat("Tümü"); }}>👤 BİREYSEL</button>
            <button className={`toggle-btn ${aktifTip === "ticari" ? "on-ticari" : ""}`} onClick={() => { setAktifTip("ticari"); setAktifKat("Tümü"); }}>🏭 TİCARİ & ENDÜSTRİYEL</button>
          </div>
          ...
          <div className="arama-box">
            ...
            />
            <button className="arama-btn" onClick={() => router.push(`/ilanlar?q=${aramaQ}&tip=${aktifTip}&rol=${aktifRol}`)}>Ara & Listele</button>
          </div>
        </div>
      </header>
      ...
      <main className="vitrin-wrap">
        <div className="vitrin-head">
          <div>
            <h2 className="vitrin-title">{aktifTip === "ticari" ? "🏭 Ticari Pazar Yeri" : "👤 Bireysel İlanlar"}</h2>
            <p className="vitrin-sub">{filtreliIlanlar.length > 0 ? `${filtreliIlanlar.length} ilan bulundu` : "Henüz ilan yok — ilk ilanı siz verin!"}</p>
          </div>
          <button className="nav-btn primary" style={{ borderRadius: "12px", padding: "10px 20px" }} onClick={() => router.push(`/ilan-ver?tip=${aktifTip}&rol=${aktifRol}`)}>
        ...
        <div className="ilan-grid">
          {yukleniyor ? (
            ...
            <div className="empty">
              ...
              <button className="nav-btn primary" style={{ borderRadius: 12, padding: "11px 28px", fontSize: ".9rem" }} onClick={() => router.push(`/ilan-ver?tip=${aktifTip}&rol=${aktifRol}`)}>Hemen İlan Ver</button>
            </div>
          ) : (
              ...
              <div key={ilan._id} className="kart" onClick={() => router.push(`/ilan/${ilan._id}`)}>
                <div className="kart-img">
                  {gorsel(ilan) ? (
                    ...
                    <span>
                      {ilan.sektorId === "turizm" ? "🏨" : ilan.sektorId === "uretim" ? "🏭" : ilan.sektorId === "giyim" ? "👗" : ilan.sektorId === "lojistik" ? "🚢" : ilan.sektorId === "gida" ? "🌾" : ilan.tip === "ticari" ? "🏭" : "📋"}
                    </span>
                  )}
                  <div className="kart-badges">
                    <span className={`badge ${ilan.tip === "ticari" ? "b-ticari" : "b-bireysel"}`}>{ilan.tip === "ticari" ? "TİCARİ" : "BİREYSEL"}</span>
                    <span className={`badge ${ilan.rol === "alan" ? "b-talep" : "b-hizmet"}`}>{ilan.rol === "alan" ? "🙋 TALEP" : "💼 HİZMET"}</span>
                ...
                <div className="kart-body">
                  ...
                  <div className="kart-foot">
                    ...
                    <button className={`kart-btn ${ilan.rol === "alan" ? "red" : ""}`} onClick={e => { e.stopPropagation(); router.push(`/ilan/${ilan._id}`); }}>
        ...
        <div className="destek-panel">
          ...
          <div className="destek-body">
            {destekGonderildi ? (
              ...
              <>
                <p style={{ fontSize: ".82rem", color: "#64748b", marginBottom: 10, lineHeight: 1.5 }}>Soru, öneri veya sorunlarınızı yazın — ekibimiz size özel yanıt verir.</p>
                <textarea className="destek-ta" placeholder="Mesajınızı yazın..." value={destekMesaj} onChange={e => setDestekMesaj(e.target.value)} />
                {session?.user?.email && <p style={{ fontSize: ".75rem", color: "#94a3b8", marginTop: 4 }}>📧 {session.user.email} üzerinden yanıtlanacak</p>}
      ...
      <nav className="bottom-nav" aria-label="Alt navigasyon">
        {BOTTOM_NAV.map(item => (
          <button
            key={item.href}
            className={`bottom-nav-item ${item.primary ? "bottom-nav-primary" : ""} ${pathname === item.href ? "active" : ""}`}
            onClick={() => {
