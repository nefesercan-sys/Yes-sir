    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .kat-wrap { max-width: 1200px; margin: 24px auto 0; padding: 0 16px; }
        .kat-scroll { display: flex; gap: 8px; overflow-x: auto; padding-bottom: 8px; scrollbar-width: none; }
        .kat-scroll::-webkit-scrollbar { display: none; }
        .kat-chip { display: inline-flex; flex-direction: column; align-items: center; gap: 6px; padding: 12px 16px; background: #fff; border: 1.5px solid var(--border); border-radius: 16px; cursor: pointer; transition: .18s; white-space: nowrap; flex-shrink: 0; min-width: 72px; }
        .kat-chip:hover { border-color: var(--blue); transform: translateY(-2px); box-shadow: 0 4px 12px rgba(37,99,235,.12); }
        .kat-chip.on { background: var(--navy); border-color: var(--navy); color: #fff; box-shadow: 0 4px 16px rgba(13,27,62,.25); }
        .kat-icon { font-size: 1.4rem; line-height: 1; }
        .kat-label { font-size: .68rem; font-weight: 700; color: var(--mid); text-align: center; line-height: 1.2; }
        .kat-chip.on .kat-label { color: rgba(255,255,255,.85); }
      ...
      <div className="kat-wrap">
        <div className="kat-scroll">
          {kategoriler.map(k => (
            ...
            >
              <span className="kat-icon">{k.icon}</span>
              <span className="kat-label">{k.id === "Tümü" ? "Tümü" : k.ad}</span>
            </button>
          ))}
        </div>
      </div>
