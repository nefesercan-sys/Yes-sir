      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {([
            ...
            { k:'sistem_ilanlari', l:'📋 Tüm İlanlar' },
            { k:'mesajlar',        l:'📩 Gelen Talepler' },
  ...
  const handleUret = async () => {
    ...
    try {
      const res = await fetch('/api/ai-ilan', {
        ...
        body: JSON.stringify({ sektorId: secilenSektor, sehir: secilenSehir === 'Rastgele' ? null : secilenSehir, adet, adminKey, tip: 'bireysel', rol: rolSecim === 'her-ikisi' ? undefined : rolSecim, yapay: true }),
      });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-5 mb-4 shadow-lg h-[300px] overflow-y-auto">
  ...
  const handleUret = async () => {
    ...
    try {
      const res = await fetch('/api/ai-ilan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sektorId: secilenSektor, sehir: 'Merkez', ulke: secilenUlke === 'Rastgele' ? null : secilenUlke, adet, adminKey, tip: 'ticari', rol: rolSecim === 'her-ikisi' ? undefined : rolSecim, yapay: true }),
      });
      const data = await res.json();
      if (data.success) { setSonuc(data); addLog(`✅ ${data.uretilen} ilan oluşturuldu!`); }
      else addLog(`❌ Hata: ${data.error}`);
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-5 mb-4 shadow-lg">
          ...
          <div className="grid grid-cols-1 gap-2">
            {([
              ...
              { v:'veren',     l:'🏭 Üretici / Tedarikçi',   a:'Müşteri çekmek için' },
              { v:'alan',      l:'📦 Alıcı / İthalatçı',     a:'Tedarikçi çekmek için' },
      ...
      <div>
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-5 mb-4 shadow-lg">
          ...
          <p className="text-yellow-500 text-[11px] mt-4 bg-yellow-500/10 p-3 rounded-lg border border-yellow-500/20 leading-relaxed">
            ⚠️ <strong>Önemli:</strong> Bu ilanlar sisteme &quot;yapay&quot; olarak işlenir.
          </p>
        </div>
        <div className="flex gap-3 mb-4">
          <button onClick={handleUret} disabled={yukleniyor || !secilenSektor}
...
function SistemIlanlariPaneli({ adminKey }: { adminKey: string }) {
  ...
  const filtrelenmis = ilanlar.filter(i => {
    if (filtre === 'gercek') return !i.yapay;
    if (filtre === 'yapay') return i.yapay;
    return true;
  });
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2 bg-gray-900 p-1 rounded-xl border border-gray-700">
          ...
          <button onClick={() => setFiltre('gercek')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${filtre==='gercek'?'bg-blue-600 text-white':'text-gray-400'}`}>Gerçek</button>
          <button onClick={() => setFiltre('yapay')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${filtre==='yapay'?'bg-purple-600 text-white':'text-gray-400'}`}>🤖 AI</button>
        </div>
        <button onClick={yukle} className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-xl text-xs font-bold border border-gray-600">
          🔄 Yenile
        </button>
      </div>
      {yukleniyor ? <p className="text-gray-500 text-sm text-center py-10">Yükleniyor...</p> :
       filtrelenmis.length === 0 ? <p className="text-gray-500 text-sm text-center py-10">İlan bulunamadı.</p> : (
        <div className="space-y-3">
          {filtrelenmis.map(i => (
            <div key={i._id} className="bg-gray-900 border border-gray-700 rounded-2xl p-4 flex gap-4 items-center hover:border-gray-500 transition-all">
              <div className="w-12 h-12 rounded-xl bg-gray-800 flex items-center justify-center shrink-0 text-xl overflow-hidden">
                {i.medyalar?.[0] ? <img src={i.medyalar[0]} className="w-full h-full object-cover" alt="" /> : '📋'}
              </div>
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <a href={`/ilan/${i._id}`} target="_blank" className="text-center bg-gray-800 hover:bg-gray-700 text-gray-300 px-3 py-1.5 rounded-lg text-[11px] font-bold">İncele</a>
                <button onClick={() => ilanSil(i._id)} className="bg-red-500/10 hover:bg-red-500/20 text-red-400 px-3 py-1.5 rounded-lg text-[11px] font-bold border border-red-500/20">Sil</button>
              </div>
            </div>
          ))}
        </div>
...
function SistemTicaretPaneli({ adminKey: _ }: { adminKey: string }) {
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 text-center">
      <span className="text-5xl mb-4 block">💸</span>
      <h2 className="text-white font-bold text-lg mb-2">Ticaret Ağ İzleyicisi</h2>
      <p className="text-gray-400 text-sm max-w-md mx-auto">Bu modül, platformdaki tüm kullanıcılar arasında dönen teklifleşmeleri listeler. API entegrasyonu sağlandığında aktif olacaktır.</p>
    </div>
  );
}

function UyelerPaneli({ adminKey: _ }: { adminKey: string }) {
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 text-center">
      <span className="text-5xl mb-4 block">👥</span>
      <h2 className="text-white font-bold text-lg mb-2">Global Üye Veritabanı</h2>
      <p className="text-gray-400 text-sm max-w-md mx-auto">Platforma kayıtlı tüm kullanıcıların listesi burada gösterilecektir.</p>
    </div>
  );
}
...
function IstatistikPaneli({ adminKey }: { adminKey: string }) {
  ...
  return (
    <div>
      <button onClick={yukle} disabled={yukleniyor}
        className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-4 rounded-xl font-black text-sm mb-6 transition-all">
        {yukleniyor ? '⏳ Çekiliyor...' : '📊 Ağ Verilerini Güncelle'}
      </button>
      {stats && (
