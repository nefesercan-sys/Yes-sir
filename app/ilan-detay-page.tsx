...
export default function IlanDetayPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) { setHata(res.status === 404 ? "İlan bulunamadı." : "İlan yüklenirken bir hata oluştu."); }
        else setIlan(await res.json());
  ...
  const handleSil = async () => {
    ...
    try {
      ...
      if (res.ok) router.push("/profil?silindi=1");
      else { const d = await res.json(); setHata(d.error || "Silme başarısız."); setSilOnay(false); }
    } catch { setHata("Bağlantı hatası."); setSilOnay(false); }
    setSiliyor(false);
  };

  const handlePaylasim = () => {
    if (navigator.share) navigator.share({ title: ilan?.baslik, url: window.location.href });
    else { navigator.clipboard.writeText(window.location.href); setToast("🔗 Bağlantı kopyalandı!"); }
  };
      ...
      <div style={{ background: "#0f172a", padding: "14px 20px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,.3)" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ color: "rgba(255,255,255,.4)", fontSize: 11, marginTop: 2 }}>
            {ilan.sehir}{ilan.ulke && ilan.ulke !== "Türkiye" ? `, ${ilan.ulke}` : ""} · {ilan.tip === "ticari" ? "Ticari" : "Bireysel"}
          </p>
        </div>
        ...
        <div>
          ...
          {medyalar.length > 1 && (
            <div style={{ display: "flex", gap: 6, padding: "8px 16px", overflowX: "auto", background: "#0f172a" }}>
              {medyalar.map((url, i) => (
                <div key={i} onClick={() => setAktifFoto(i)} style={{ width: 52, height: 52, borderRadius: 8, overflow: "hidden", border: `2px solid ${i === aktifFoto ? "#f59e0b" : "transparent"}`, cursor: "pointer", flexShrink: 0, transition: "border .15s" }}>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "16px 16px" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 700, background: ilan.rol === "alan" ? "#f0fdf4" : "#fff1f2", color: ilan.rol === "alan" ? "#166534" : "#9f1239", padding: "3px 9px", borderRadius: 6, border: `1px solid ${ilan.rol === "alan" ? "#bbf7d0" : "#fecdd3"}` }}>
              {ilan.rol === "alan" ? "🛒 Talep" : "💼 Hizmet Veren"}
            </span>
            <span style={{ fontSize: 10, fontWeight: 700, background: "#f8fafc", color: "#475569", padding: "3px 9px", borderRadius: 6, border: "1px solid #e2e8f0" }}>
              📂 {ilan.kategori}
            </span>
          </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 24, width: "100%", maxWidth: 420, animation: "fadeIn .2s ease" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleSil} disabled={siliyor} style={{ flex: 1, padding: 13, borderRadius: 12, background: siliyor ? "#94a3b8" : "#dc2626", border: "none", color: "#fff", fontFamily: "inherit", fontSize: 14, fontWeight: 800, cursor: siliyor ? "not-allowed" : "pointer" }}>
