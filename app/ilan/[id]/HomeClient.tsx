...
function getImageUrl(ilan: any): string {
  ...
  if (img) return img;
  for (const key of ["resimler", "medyalar", "images"]) {
    if (typeof ilan[key] === "string" && ilan[key].length > 5) return ilan[key];
  }
...
const JSON_LD = {
  ...
  potentialAction: {
    "@type": "SearchAction",
    target: "https://atakasa.com/kesfet?q={search_term_string}",
...
interface KartiProps {
  ilan: any;
  index: number;
  onOpenModal: (ilan: any, tur: "takas" | "satinal") => void;
  onSepeteEkle: (ilan: any) => void;
  onVideoOpen: (url: string, baslik: string) => void;
}

const BorsaKarti = memo(function BorsaKarti({
  ilan, index, onOpenModal, onSepeteEkle, onVideoOpen,
}: KartiProps) {
  const router = useRouter();
  const ilkMedya = getImageUrl(ilan);
  const videoVar = isVideoUrl(ilkMedya);
  const pozitif = (ilan.degisimYuzdesi || 0) >= 0;
  const optimizedSrc = videoVar ? ilkMedya : optimizeCloudinary(ilkMedya, 520, 220);
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: `${ilan.baslik} | A-TAKASA`, text: "Bu ilana bakmalısın!", url: shareUrl }); }
      catch {}
    } else {
  ...
  return (
    <div className="product-card" itemScope itemType="https://schema.org/Product">
      ...
      {videoVar && <div className="video-badge"><Play size={8} fill="currentColor" /> VİDEO</div>}

      <div
        className="card-media"
        onClick={() => {
          if (videoVar) onVideoOpen(ilkMedya, ilan.baslik || "");
      ...
      >
        {videoVar ? (
          ...
          <img
            ...
            itemProp="image"
            onError={(e) => {
              (e.target as HTMLImageElement).src = "https://placehold.co/520x220/1e3a5f/c9a84c?text=A-TAKASA";
            }}
          />
        )}
      ...
      <div className="card-body">
        ...
        <div className="card-actions">
          <div className="action-row-top">
            ...
            <button onClick={() => onSepeteEkle(ilan)} className="btn-cart" aria-label="Sepete ekle">
              <ShoppingCart size={15} />
            </button>
          </div>
          <div className="action-row-bottom">
            <button onClick={() => onOpenModal(ilan, "takas")} className="btn-swap">🔄 Takas Teklifi</button>
            <button onClick={() => onOpenModal(ilan, "satinal")} className="btn-buy">Satın Al</button>
          </div>
        </div>
      </div>
    </div>
...
export default function HomeClient({ ilkIlanlar }: Props) {
  ...
  // ✅ SSR'dan gelen initial veri ile başla — loading ekranı geçilir
  const [ilanlar, setIlanlar] = useState<any[]>(ilkIlanlar);
  const [loading, setLoading] = useState(ilkIlanlar.length === 0);
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem("atakasa_sepet");
      if (v) JSON.parse(v);
    } catch { localStorage.removeItem("atakasa_sepet"); }
  }, []);

  // Geri kalan ilanları arka planda yükle (skip=20)
  useEffect(() => {
    const kalanYukle = async () => {
      try {
        // Eğer SSR'dan hiç veri gelmediyse tamamını çek
        if (ilkIlanlar.length === 0) {
          const res = await fetch("/api/varliklar?limit=20&durum=aktif");
        ...
        // Geri kalan ilanları lazy yükle
        setTimeout(async () => {
          try {
            const res2 = await fetch("/api/varliklar?limit=200&skip=20&durum=aktif");
    ...
    if (session?.user?.email && ilanlar.length > 0) {
      ...
      setBenimIlanlarim(
        ilanlar.filter((i: any) =>
          (i.satici?.email || i.sellerEmail || i.satici || "").toString().toLowerCase() === kEmail
        )
      );
    }
  }, [session, ilanlar]);
  ...
  const handleVideoOpen = useCallback((url: string, baslik: string) => {
    setVideoModalUrl(url); setVideoModalBaslik(baslik);
  }, []);
    ...
    <div className="at-root">
      ...
      <nav className="top-nav" aria-label="Ana navigasyon">
        <div className="nav-inner">
          <div onClick={() => router.push("/")} style={{ cursor: "pointer", marginRight: 16, display: "flex", alignItems: "center" }} role="link" aria-label="Ana sayfaya git">
            <h1 style={{ color: "var(--navy)", fontSize: 24, fontWeight: 800, fontFamily: "var(--font-display)", letterSpacing: "-0.02em", margin: 0 }}>
              A-TAKASA<span style={{ color: "var(--gold)" }}>.</span>
            </h1>
          </div>
          <div className="search-wrap">
            <Search size={17} className="search-icon" aria-hidden="true" />
            <input className="search-input" placeholder="Varlık ara..." value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)} aria-label="Varlık ara" type="search" />
          </div>
          <div className="nav-actions">
            <button onClick={() => setFiltreMenusuAcik(!filtreMenusuAcik)}
              className={`btn-filter ${filtreMenusuAcik ? "active" : ""}`} aria-expanded={filtreMenusuAcik}>
          ...
          <div className="filter-panel">
            <div className="filter-grid">
              ...
              <div className="filter-field">
                ...
                <input id="min-fiyat" type="number" placeholder="Örn: 1000" value={minFiyat}
                  onChange={(e) => setMinFiyat(e.target.value)} className="filter-input" min="0" />
              </div>
              <div className="filter-field">
                <label className="filter-label" htmlFor="max-fiyat">Max Fiyat (₺)</label>
                <input id="max-fiyat" type="number" placeholder="Örn: 50000" value={maxFiyat}
                  onChange={(e) => setMaxFiyat(e.target.value)} className="filter-input" min="0" />
              </div>
              <div className="filter-field" style={{ justifyContent: "flex-end" }}>
                <button onClick={() => setSadeceTakaslik(!sadeceTakaslik)}
                  className={`swap-toggle ${sadeceTakaslik ? "active" : ""}`} aria-pressed={sadeceTakaslik}>
      ...
      <div className="cat-strip" role="navigation" aria-label="Kategori filtresi">
        <div className="cat-inner">
          <button onClick={() => { setAktifKategori("Hepsi"); setAktifAltFiltre("Yeni İlanlar"); }}
            className={`cat-btn ${aktifKategori === "Hepsi" ? "active" : ""}`} aria-pressed={aktifKategori === "Hepsi"}>
            🌐 Tümü
          </button>
          {SEKTORLER.map((s) => (
            <button key={s.ad} onClick={() => { setAktifKategori(s.ad); setAktifAltFiltre("Yeni İlanlar"); }}
              className={`cat-btn ${aktifKategori === s.ad ? "active" : ""}`} aria-pressed={aktifKategori === s.ad}>
              {s.emoji} {s.ad}{" "}
        ...
        <div className="sub-filter-bar" role="navigation" aria-label="Alt filtreler">
          {["Yeni İlanlar", "En Çok Fiyatı Düşenler", "En Çok Yükselenler", "En Çok Takas Edilenler"].map((f) => (
            <button key={f} onClick={() => setAktifAltFiltre(f)}
              className={`sub-filter-btn ${aktifAltFiltre === f ? "active" : ""}`} aria-pressed={aktifAltFiltre === f}>
              {f}
            </button>
          ))}
        </div>
          ...
          <div className="product-grid">
            {filtrelenmisIlanlar.map((ilan, index) => (
              <BorsaKarti key={ilan._id} ilan={ilan} index={index}
                onOpenModal={openModal} onSepeteEkle={handleSepeteEkle} onVideoOpen={handleVideoOpen} />
            ))}
          </div>
        ) : (
        ...
        <div className="modal-overlay" onClick={closeModal} role="dialog" aria-modal="true">
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <img src={optimizeCloudinary(getImageUrl(seciliIlan), 80, 80)} className="modal-img"
                alt={seciliIlan.baslik || "Ürün"} loading="lazy" width={80} height={80} />
            ...
            {modalTuru === "takas" ? (
              <div className="modal-form">
                ...
                <input id="nakit-ekle" type="number" placeholder="Örn: 5000" value={eklenecekNakit}
                  onChange={(e) => setEklenecekNakit(e.target.value)} className="form-input" min="0" />
                <button onClick={handleTakasGonder} disabled={!secilenBenimIlanim}
                  className="btn-modal-primary" style={{ marginTop: 16 }}>
              ...
              <div className="modal-form">
                ...
                <input type="tel" placeholder="Telefon Numarası" value={siparisForm.telefon}
                ...
                <textarea placeholder="Teslimat Adresi" value={siparisForm.adres}
                  ...
                  className="form-textarea" autoComplete="street-address" />
                <select value={siparisForm.odemeYontemi}
                  onChange={(e) => setSiparisForm({ ...siparisForm, odemeYontemi: e.target.value })} className="form-select">
                ...
                <div className="legal-box">
                  <label className="legal-check">
                    ...
                    <span><strong>📄 Kullanıcı Sözleşmesi:</strong> Satış koşullarını okudum, kabul ediyorum.</span>
                  </label>
                  <label className="legal-check" style={{ marginTop: 8 }}>
      ...
      {videoModalUrl && (
        <div className="modal-overlay" onClick={() => setVideoModalUrl(null)} role="dialog" aria-modal="true">
          <div className="modal-box" onClick={(e) => e.stopPropagation()}
