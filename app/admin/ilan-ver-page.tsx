...
// ✅ DÜZELTİLDİ:
//   1. status useSession'dan alındı
//   2. Fiyat bloğu adım 2 içine taşındı
//   3. canUploadMore mantığı düzeltildi
//   4. Adım 2→3 geçişinde validasyon eklendi
// ============================================================
import { useState, useEffect, Suspense } from "react";
export const dynamic = "force-dynamic";
import { useSession } from "next-auth/react";
import { useRouter, useSearchParams } from "next/navigation";
import { BIREYSEL_SEKTORLER, TICARI_SEKTORLER, Sektor, FormAlan } from "@/lib/sektorler";
...
function IlanVerIcerik() {
  // ✅ DÜZELTME 1: status de destructure edildi
  ...
  const aktifForm = seciliSektor
    ? rol === "alan"
      ? seciliSektor.hizmetAlanFormu
      : seciliSektor.hizmetVerenFormu
    : [];
  ...
  // ✅ DÜZELTME 2: Adım 2→3 validasyonlu geçiş
  const handleAdim2Devam = () => {
    setHata("");
  ...
  const handleYayinla = async () => {
    ...
    try {
      ...
      if (res.ok) router.push(`/ilan/${data.id}?yeni=1`);
      else setHata(data.error || "Bir hata oluştu");
    } catch {
        ...
        return (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
            {alan.secenekler?.map(s => {
              ...
              return (
                <button key={s} type="button" onClick={() => toggleMulti(alan.key, s)} style={{ padding: "7px 12px", borderRadius: 8, border: `1.5px solid ${secili ? "#2563eb" : "#e2e8f0"}`, background: secili ? "#2563eb" : "#fff", color: secili ? "#fff" : "#475569", fontFamily: "inherit", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                  {secili ? "✓ " : ""}{s}
                </button>
              );
            })}
  ...
  // ✅ DÜZELTME 3: Resim ve video ayrı ayrı kontrol edilir
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ background: "#0f172a", padding: "14px 20px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,.3)" }}>
        <button onClick={() => adim > 1 ? setAdim(p => p - 1) : router.push("/")} style={{ background: "rgba(255,255,255,.1)", border: "none", color: "#fff", width: 38, height: 38, borderRadius: 10, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>←</button>
        ...
        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
          {[1, 2, 3].map(a => <div key={a} style={{ width: 32, height: 4, borderRadius: 2, background: a <= adim ? "#f59e0b" : "rgba(255,255,255,.15)", transition: ".2s" }} />)}
        </div>
      </div>
          ...
          <div>
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                {(["bireysel", "ticari"] as const).map(t => (
                  <button key={t} onClick={() => setTip(t)} style={{ flex: 1, padding: "11px 8px", borderRadius: 10, border: `2px solid ${tip === t ? (t === "ticari" ? "#f59e0b" : "#2563eb") : "#e2e8f0"}`, background: tip === t ? (t === "ticari" ? "#fffbeb" : "#eff6ff") : "#fff", color: tip === t ? (t === "ticari" ? "#92400e" : "#1d4ed8") : "#64748b", fontFamily: "inherit", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {(["alan", "veren"] as const).map(r => (
                  <button key={r} onClick={() => setRol(r)} style={{ flex: 1, padding: "11px 8px", borderRadius: 10, border: `2px solid ${rol === r ? (r === "alan" ? "#0f172a" : "#dc2626") : "#e2e8f0"}`, background: rol === r ? (r === "alan" ? "#0f172a" : "#dc2626") : "#fff", color: rol === r ? "#fff" : "#64748b", fontFamily: "inherit", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(145px, 1fr))", gap: 10 }}>
              {gosterilecekSektorler.map(s => (
                <div key={s.id} onClick={() => { setSeciliSektor(s); setFormData({}); setAdim(2); }} style={{ background: "#fff", borderRadius: 16, border: `2px solid ${seciliSektor?.id === s.id ? s.renk : "#e2e8f0"}`, padding: "18px 12px", cursor: "pointer", textAlign: "center", transition: "all .15s", boxShadow: seciliSektor?.id === s.id ? `0 4px 16px ${s.renk}30` : "none" }} onMouseEnter={e => (e.currentTarget.style.borderColor = s.renk)} onMouseLeave={e => { if (seciliSektor?.id !== s.id) e.currentTarget.style.borderColor = "#e2e8f0"; }}>
        ...
        {adim === 2 && seciliSektor && (
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, padding: 16, background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0" }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: seciliSektor.renk + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>{seciliSektor.icon}</div>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", fontFamily: "Unbounded, sans-serif" }}>{seciliSektor.ad}</h2>
                <p style={{ color: "#94a3b8", fontSize: 12, marginTop: 2 }}>{rol === "alan" ? "📋 Talep İlanı — Ne arıyorsunuz?" : "💼 Hizmet / Ürün İlanı — Ne sunuyorsunuz?"}</p>
              </div>
              <button onClick={() => setAdim(1)} style={{ marginLeft: "auto", background: "#f1f5f9", border: "none", padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600, color: "#475569", fontFamily: "inherit", flexShrink: 0 }}>Değiştir</button>
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 14 }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 6 }}>Başlığı kısa ve net tutun.</p>
              </div>
            ...
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea value={formData.aciklama || ""} onChange={e => setField("aciklama", e.target.value)} placeholder="Sunacağınız hizmeti, aradığınız ürünü veya özel şartlarınızı detaylıca anlatın..." rows={5} style={{ ...INP, resize: "vertical", minHeight: 120 }} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
            ...
            {Object.entries(gruplar).map(([grupAdi, alanlar]) => (
              <div key={grupAdi} style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 14 }}>
                <h3 style={{ fontSize: 11, fontWeight: 800, color: seciliSektor.renk, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 14, paddingBottom: 10, borderBottom: `2px solid ${seciliSektor.renk}20` }}>{grupAdi}</h3>
            ...
            {/* ✅ DÜZELTME 4: Fiyat bloğu adım 2 içinde */}
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[{ key: "butceMin", label: "Minimum Fiyat" }, { key: "butceMax", label: "Maksimum Fiyat" }].map(item => (
                  <div key={item.key}>
              ...
              <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 8 }}>💡 Fiyat aralığı belirtmek daha fazla teklif almanızı sağlar.</p>
            </div>

            <button onClick={handleAdim2Devam} style={{ width: "100%", padding: 15, borderRadius: 14, background: "#2563eb", border: "none", color: "#fff", fontFamily: "inherit", fontSize: 15, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 16px rgba(37,99,235,.3)" }}>
        ...
        {adim === 3 && seciliSektor && (
          <div>
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 16 }}>
              ...
              <p style={{ color: "#94a3b8", fontSize: 12, marginBottom: 16, lineHeight: 1.6 }}>Görsel eklemek teklif alma oranınızı <strong>3 kat</strong> artırır. Maksimum <strong>10 fotoğraf</strong> ve <strong>1 video</strong>.</p>
              ...
              {canUploadMore ? (
                ...
                <div style={{ padding: 12, borderRadius: 10, background: "#f0fdf4", border: "1px solid #bbf7d0", color: "#166534", fontSize: 12, textAlign: "center", fontWeight: 600 }}>✅ Maksimum medya sayısına ulaşıldı (10 Fotoğraf + 1 Video).</div>
              )}
            </div>
            ...
            <div style={{ background: "#f8fafc", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  { l: "Fiyat", v: (formData.butceMin || formData.butceMax) ? `${formData.butceMin || 0} — ${formData.butceMax || 0} ${seciliSektor.butceBirimi || "₺"}` : undefined },
                  { l: "Medya", v: medyalar.length > 0 ? `${resimSayisi} fotoğraf${videoSayisi > 0 ? " + 1 video" : ""}` : "Yok" },
                ].filter(r => r.v).map(r => (
            ...
            <button onClick={handleYayinla} disabled={yukleniyor} style={{ width: "100%", padding: 16, borderRadius: 14, background: yukleniyor ? "#94a3b8" : "#f59e0b", border: "none", color: yukleniyor ? "#fff" : "#0f172a", fontFamily: "inherit", fontSize: 15, fontWeight: 800, cursor: yukleniyor ? "not-allowed" : "pointer", boxShadow: yukleniyor ? "none" : "0 4px 20px rgba(245,158,11,.4)", transition: ".2s" }}>
