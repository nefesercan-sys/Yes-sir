...
export async function GET(_: NextRequest, { params }: Props) {
  ...
  } catch { return NextResponse.json({ error: 'Geçersiz ID' }, { status: 400 }); }
}
...
export async function PATCH(req: NextRequest, { params }: Props) {
  ...
  await db.collection('ilanlar').updateOne({ _id: new ObjectId(params.id) }, { $set: { ...body, guncellendi: new Date() } });
