...
export async function GET(req: NextRequest) {
  ...
  const filter: any = { durum: 'aktif' };
  if (sektor) filter.sektorId = sektor;
  if (q) filter.$text = { $search: q };
  if (sehir) filter['formData.sehir'] = { $regex: sehir, $options: 'i' };

  const ilanlar = await db.collection('ilanlar').find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).toArray();
...
export async function POST(req: NextRequest) {
  ...
  const { sektorId, baslik, formData, medyalar, butceMin, butceMax, butceBirimi, gizliAd } = body;

  if (!sektorId || !baslik) return NextResponse.json({ error: 'Sektör ve başlık zorunlu' }, { status: 400 });
  ...
  const ilan = {
    ...
    sahibi: session ? { email: session.user?.email, ad: session.user?.name } : null,
