...
type Tab = 'ozet' | 'ilanlarim' | 'tekliflerim' | 'rezervasyonlar' | 'bildirimler' | 'profil' | 'ayarlar';
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: '#0f172a', padding: '11px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <img src={session.user?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(session.user?.name || 'U')}&background=1e3a5f&color=f59e0b`}
            alt="" style={{ width: '30px', height: '30px', borderRadius: '50%', border: '2px solid rgba(245,158,11,0.4)' }} />
      ...
      <div className="panel-layout">
        ...
        <div className="sidebar">
          <div style={{ marginBottom: '18px', padding: '12px', background: '#f8fafc', borderRadius: '12px' }}>
            <img src={session.user?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(session.user?.name || 'U')}&background=1e3a5f&color=f59e0b`}
              alt="" style={{ width: '42px', height: '42px', borderRadius: '50%', marginBottom: '7px', display: 'block' }} />
          ...
          <div style={{ marginTop: '14px', paddingTop: '14px', borderTop: '1px solid #e2e8f0' }}>
            <button onClick={() => router.push('/ilan-ver')}
              style={{ width: '100%', padding: '10px', borderRadius: '10px', background: '#f59e0b', border: 'none', color: '#0f172a', fontFamily: 'inherit', fontSize: '12px', fontWeight: '700', cursor: 'pointer', marginBottom: '8px' }}>
              ⚡ Yeni İlan Ver
            </button>
            {!profil && (
              <button onClick={() => router.push('/otel-profil')}
                style={{ width: '100%', padding: '10px', borderRadius: '10px', background: '#7c3aed', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>
                🏨 Tesis Profili Oluştur
              </button>
            )}
          </div>
        </div>
            ...
            <div>
              ...
              {!profil && (
                <div style={{ padding: '16px', borderRadius: '14px', background: '#fef9c3', border: '1px solid #fde68a', marginBottom: '16px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  <button onClick={() => router.push('/otel-profil')}
                    style={{ marginLeft: 'auto', padding: '8px 14px', borderRadius: '8px', background: '#f59e0b', border: 'none', color: '#0f172a', fontFamily: 'inherit', fontSize: '12px', fontWeight: '700', cursor: 'pointer', flexShrink: 0 }}>
                    Tamamla →
                  </button>
                </div>
              )}
          ...
          {aktifTab === 'ilanlarim' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <button onClick={() => router.push('/ilan-ver')}
                  style={{ padding: '8px 16px', borderRadius: '10px', background: '#0f172a', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>
                  + Yeni İlan
                </button>
              </div>
                  ...
                  <div key={i._id} className="row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a', marginBottom: '4px' }}>
                        {i.butceMin > 0 ? `${i.butceMin.toLocaleString()} ${i.butceBirimi}` : 'Bütçe açık'}
                      </p>
                      ...
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        <button onClick={() => router.push(`/ilan/${i._id}`)}
                          style={{ padding: '5px 10px', borderRadius: '7px', background: '#f1f5f9', border: 'none', color: '#475569', fontFamily: 'inherit', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
                          Teklifleri Gör
                        </button>
                        <button onClick={async () => {
                          await fetch(`/api/ilanlar/${i._id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ teklifeAcik: !i.teklifeAcik }) });
                          yukle();
                        }} style={{ padding: '5px 10px', borderRadius: '7px', background: i.teklifeAcik ? '#fef9c3' : '#ecfdf5', border: 'none', color: i.teklifeAcik ? '#92400e' : '#059669', fontFamily: 'inherit', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
                          {i.teklifeAcik ? '🟡 Kapat' : '🟢 Aç'}
                        </button>
                        <button onClick={async () => {
                          if (!confirm('İlanı silmek istediğinize emin misiniz?')) return;
                          await fetch(`/api/ilanlar/${i._id}`, { method: 'DELETE' });
                          yukle();
                        }} style={{ padding: '5px 10px', borderRadius: '7px', background: '#fef2f2', border: 'none', color: '#dc2626', fontFamily: 'inherit', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
                          Sil
                        </button>
                      </div>
                    </div>
                ...
                teklifler.length === 0 ? (
                  ...
                  <div key={t._id} className="row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '17px', fontWeight: '800', color: '#0f172a', marginBottom: '3px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>
                      ...
                      <button onClick={() => router.push(`/ilan/${t.ilanId}`)}
                        style={{ padding: '5px 10px', borderRadius: '7px', background: '#eff6ff', border: 'none', color: '#2563eb', fontFamily: 'inherit', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
                        İlana Git
                      </button>
                    </div>
                  </div>
                  ...
                  <div key={r._id} className="row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: '13px', fontWeight: '600', color: '#0f172a', marginBottom: '2px' }}>
                        {r.musteri?.email === session.user?.email ? `Hizmet Alan: ${r.hizmetVeren?.ad}` : `Müşteri: ${r.musteri?.ad}`}
                      </p>
                      <p style={{ fontSize: '17px', fontWeight: '800', color: '#059669', marginBottom: '3px' }}>
                        {Number(r.fiyat).toLocaleString()} {r.doviz}
                      </p>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                {bildirimler.some(b => !b.okundu) && (
                  <button onClick={tumunuOku} style={{ padding: '7px 14px', borderRadius: '8px', background: '#f1f5f9', border: 'none', color: '#475569', fontFamily: 'inherit', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
              ...
              {loading ? <p style={{ color: '#94a3b8' }}>Yükleniyor...</p> :
                bildirimler.length === 0 ? (
                  ...
                  <div key={b._id}
                    onClick={() => !b.okundu && bildirimOku(b._id)}
                    style={{ background: b.okundu ? 'white' : '#eff6ff', border: `1.5px solid ${b.okundu ? '#e2e8f0' : '#bfdbfe'}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '8px', cursor: b.okundu ? 'default' : 'pointer', transition: 'all 0.15s' }}>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                      <span style={{ fontSize: '20px', flexShrink: 0 }}>
                        {b.tip === 'yeni_teklif' ? '💼' : b.tip === 'teklif_kabul' ? '🎉' : b.tip === 'yeni_siparis' ? '🛍️' : '🔔'}
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {b.ilanId && (
                          <button onClick={e => { e.stopPropagation(); router.push(`/ilan/${b.ilanId}`); }}
                            style={{ marginTop: '6px', padding: '4px 10px', borderRadius: '6px', background: '#0f172a', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '10px', fontWeight: '600', cursor: 'pointer' }}>
                            İlana Git →
                          </button>
                        )}
                      </div>
            ...
            <div>
              ...
              {profil ? (
                <div>
                  <div className="card" style={{ marginBottom: '14px' }}>
                    <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '16px' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '12px', color: profil.onaylandi ? '#059669' : '#d97706', fontWeight: '600', marginTop: '2px' }}>
                          {profil.onaylandi ? '✅ Profil Onaylı' : '⏳ Onay Bekliyor'}
                        </p>
                      </div>
                    </div>
                  ...
                  <button onClick={() => router.push('/otel-profil')}
                    style={{ padding: '11px 24px', borderRadius: '12px', background: '#7c3aed', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                    ✏️ Profili Güncelle
                  </button>
                </div>
              ) : (
                <div className="empty">
                  ...
                  <button onClick={() => router.push('/otel-profil')}
                    style={{ padding: '11px 28px', borderRadius: '12px', background: '#7c3aed', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                    🏨 Tesis Profili Oluştur
                  </button>
                </div>
              )}
            ...
            <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[
                    ...
                    <div key={f.k}>
                      ...
                      <input type={f.t} defaultValue={f.v} disabled={f.d}
                        style={{ width: '100%', padding: '11px 14px', borderRadius: '11px', border: '1.5px solid #e2e8f0', fontSize: '14px', fontFamily: 'inherit', background: f.d ? '#f8fafc' : 'white', color: f.d ? '#94a3b8' : '#0f172a' }} />
                    </div>
                  ))}
                  <button style={{ padding: '11px', borderRadius: '11px', background: '#0f172a', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                    Kaydet
                  </button>
                </div>
              </div>
              ...
              <div className="card" style={{ background: '#fef2f2', borderColor: '#fecaca' }}>
                ...
                <button onClick={() => signOut({ callbackUrl: '/' })}
                  style={{ padding: '10px 20px', borderRadius: '10px', background: '#dc2626', border: 'none', color: 'white', fontFamily: 'inherit', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}>
                  Çıkış Yap
                </button>
              </div>
            </div>
